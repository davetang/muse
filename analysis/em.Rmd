...
| Step | What you do | Two-coin example | Gaussian mixture example |
|------|-------------|------------------|--------------------------|
| **Hidden data** | Identify what is unobserved | Which coin was used in each round | Which component generated each data point |
| **Initialise** | Make an initial guess for the parameters | $\theta_A = 0.6$, $\theta_B = 0.5$ | Means, SDs, and weights |
| **E-step** | Compute the expected value of the hidden data, given current parameters | P(coin A \| round data) for each round | P(component $k$ \| data point) for each point |
| **M-step** | Re-estimate parameters, treating the expected hidden data as if it were real | Weighted proportion of heads for each coin | Weighted mean, SD, and proportion for each component |
