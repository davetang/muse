...
```{r os_status}
head(colData(ov_tcga)$OS_STATUS)
...
Data for living patients.

```{r os_months_living}
colData(ov_tcga) |>
  as.data.frame() |>
  dplyr::filter(OS_STATUS == "0:LIVING") |>
...
col_dat$OS_YEARS <- as.numeric(col_dat$OS_MONTHS) / 12
...
```{r survfit_grade_years, fig.width=8, fig.height=6}
fit2 <- survfit(
    Surv(OS_YEARS, as.numeric(substr(OS_STATUS, 1, 1))) ~ GRADE,
    data = colData(ov_tcga)
)
ggsurvplot(fit2, data = colData(ov_tcga), risk.table = TRUE)
```
