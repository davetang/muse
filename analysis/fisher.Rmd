...
* Expected cell frequencies are low (< 5 in any cell)
* You need precise p-values, i.e., an exact test, without relying on large-sample approximations
...
* Any cell has an **expected count** less than 5
* Total sample size less than 20-40
* The test works for any sample size, but may be overkill for large datasets
...
The observed counts are 8, 3, 2, 7 and the expected counts are what you would expect if there were no association between the groups and outcomes.

Expected Count = (Row Total * Column Total) / Grand Total.

```{r eg1_expected_count}
...
* Cell (Group A, Success): (10 × 11) / 20 = 5.5
* Cell (Group A, Failure): (10 × 9) / 20 = 4.5
* Cell (Group B, Success): (10 × 11) / 20 = 5.5
* Cell (Group B, Failure): (10 × 9) / 20 = 4.5

Since two cells have expected counts < 5, Fisher's Exact Test is appropriate for this data. The chi-square test approximation relies on expected frequencies being large enough (>= 5). When expected counts are small, the chi-square approximation becomes unreliable, and Fisher's Exact Test should be used instead.

Calculate expected counts with a Chi-square test.

```{r chisq_test}
chisq_test <- chisq.test(eg1)
chisq_test$expected
```

## Clinical trial

Imagine a small clinical trial to test a new drug for migraines. 20 patients are recruited and randomly assigned to whether they will receive the new drug or a placebo. After the treatment, whether patient experienced relief from their migraine was recorded.
...
We can use Fisher's Exact Test to test "What is the probability of seeing a pattern this strong (or stronger) if the drug had no effect?"
...
```{r fisher_test_clinical_data}
res <- fisher.test(clinical_data)
res
```

If the drug had no effect, there's only about a 0.06978% chance you'd see a pattern this extreme or more extreme by random chance.

## Exact test

Unlike other statistical tests that use approximations, Fisher's Exact test:

* Counts every single possible outcome
* Calculates precise probabilities for each
* No approximations

## Comparing with Chi-Square Test

Let's see what happens if we use chi-square test (an approximation) on our small dataset:

```{r chisq_test_clinical_data}
chisq_result <- chisq.test(clinical_data)
chisq_result
```

## Global test

If you have more than two categories, Fisher's Exact Test becomes a generalised Fisher's Exact Test (also known as Fisher-Freeman-Halton test), because the regular Fisher's test is for 2x2 tables.
