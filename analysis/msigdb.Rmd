...
```{r save_msigdbhs}
outfile <- paste0(msigdb_hs_latest$title, '.rds')
saveRDS(object = msigdb.hs, file = outfile)
```
...
```{r wanted}
wanted <- sapply(lapply(msigdb.hs, collectionType), bcCategory) == "h"
table(wanted)
```
