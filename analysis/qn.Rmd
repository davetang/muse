...
When there are more ties, `normalize.quantiles` is implemented differently than my (most likely mis-)implementation.
...
I am not quite sure what `normalize.quantiles` is doing and probably need to inspect the [source](https://github.com/bmbolstad/preprocessCore). I thought the averaging approach was to average the means but this does not seem to be the case.

```{r mean_of_mean}
sum(quantile_normalisation(eg3, ties = "average")$mean) / 5
```
