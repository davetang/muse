...
my_packages <- c('tidyverse', 'tidymodels', 'broom.mixed', 'dotwhisker', 'rstanarm', 'mlbench', 'ranger', 'randomForest')
...
Load and [split data](https://rsample.tidymodels.org/reference/initial_split.html).

```{r sonar}
data(Sonar, package = "mlbench")

sonar_split <- initial_split(data = Sonar, prop = 0.8, strata = 'Class')
train_data <- training(sonar_split)
test_data <- testing(sonar_split)
```
...
rand_forest(mtry = my_mtry, trees = 500) %>%
  ...
  fit(Class ~ ., data = train_data) -> rf_ranger$model

rf_randomForest <- list()
rand_forest(mtry = my_mtry, trees = 500) %>%
  set_engine("randomForest") %>%
  set_mode("classification") %>%
  fit(Class ~ ., data = train_data) -> rf_randomForest$model
```
...
```{r predict}
predict_wrapper <- function(model, test_data, type = 'prob'){
  predict(model, test_data, type = type) %>%
...
rf_ranger$predictions <- predict_wrapper(rf_ranger$model, test_data)
rf_randomForest$predictions <- predict_wrapper(rf_randomForest$model, test_data)
```
